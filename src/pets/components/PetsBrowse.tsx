...
import { AddPetModal } from "@/pets/modals/AddPetModal"
...
export function BrowsePets({ pets, onAddPet }: BrowsePetsProps) {
    ...
    <div className="space-y-6 animate-in fade-in-0 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredPets.map((pet, index) => (
          <Card
            key={pet.id}
            className="group overflow-hidden transform transition-all duration-500 hover:scale-[1.03] hover:shadow-2xl animate-in fade-in-0 slide-in-from-bottom-4 bg-white border-0 shadow-lg hover:shadow-orange-100"
            ...
            <CardContent className="p-5">
              <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-gray-100">
                  <p className="text-xs text-gray-400">Added on {new Date(pet.dateAdded).toLocaleDateString()}</p>
                </div>
              </div>
            </CardContent>
          </Card>
